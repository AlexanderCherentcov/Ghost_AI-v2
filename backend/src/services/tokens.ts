...
export async function checkAndDeduct(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    // Расширено по решению Александра (2026-09-21): на бесплатном тарифе доступен ТОЛЬКО чат —
    // картинки, видео и музыка закрыты целиком, а не только видео. Раньше три модели картинок в
    // реестре имели minPlan FREE, и бесплатный пользователь с Caspers на балансе (промокод,
    // ручное начисление) мог их использовать, что расходилось с задуманным. Единая точка проверки:
    // через checkAndDeduct проходят все платные генерации (сайт и бот).
    // (чат к этой точке уже обработан выше и вышел из функции — здесь только image/video/music)
    if (plan === 'FREE') {
      throw Object.assign(
        new Error('На бесплатном тарифе доступен только чат — картинки, видео и музыка открываются на платных тарифах'),
        { code: 'FREE_LOCKED' },
      );
    }
