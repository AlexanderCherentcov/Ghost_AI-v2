  ...
  const worker = new Worker<ReelJob>(
    ...
    async (job: Job<ReelJob>) => {
      ...
      try {
        ...
        // 2026-08-29: механизм добавлен по прямому указанию Александра после
        // реального падения Sora/Veo от перегрузки GoAPI ("нельзя терять
        // клиентов") — на тот момент у них не было fallbackModelId вообще.
        // С тех пор Sora/Veo обеим получили fallbackModelId в реестре (см.
        // config/models.ts) — без него не остались ни они, ни другие модели
        // каталога (кроме Kling — сами являются целью фолбэков, дальше
        // отступать некуда). При сбое подставляем Kling Pro/Std того же ценового уровня
