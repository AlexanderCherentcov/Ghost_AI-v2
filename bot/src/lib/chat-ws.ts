...
export function streamChat(
  ...
  // Раньше 90с — жёсткий потолок на ВЕСЬ ответ (не "нет прогресса", а именно
  // общее время), в отличие от сайта (lib/socket.ts), где таймаут — это
  // детектор зависания между токенами (сбрасывается на каждом новом токене),
  // а не ограничение на суммарную длину ответа. Цепочка резервных моделей
  // теперь честно перебирает несколько моделей до первого токена (см.
  // backend/src/services/providers/openrouter.ts) — каждая попытка может
  // занять несколько секунд до отказа, и на длинных ответах (think-режим)
  // общее время легко подбиралось к 90с без единого реального зависания.
  timeoutMs = 180_000,
