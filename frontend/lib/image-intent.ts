...
//
// ВАЖНО: "нарисуй" и "сделай" сюда специально НЕ входят, хотя формально это
// тоже глаголы действия — это ещё и самые обычные слова для ЛЮБОГО нового
// промта ("нарисуй кота" — не правка, это независимый новый запрос). Раньше
// они были в списке, и почти каждый новый промт в домене картинок ошибочно
// уходил как редактирование предыдущей картинки (sourceImageUrl подставлялся,
// хотя пользователь хотел с нуля). Остальные глаголы ниже однозначны — их не
// используют, чтобы попросить нарисовать что-то с чистого листа.
export const EDIT_VERBS = [
  'измени', 'изменить', 'отредактируй', 'отредактировать', 'поменяй', 'поменять',
  'добавь', 'добавить', 'убери', 'убрать', 'замени', 'заменить', 'преврати', 'превратить',
  'перекрась', 'раскрась', 'стилизуй',
  'исправь', 'исправить', 'поправь', 'поправить', 'почини', 'починить',
  'edit', 'change', 'modify', 'transform', 'fix',
